...
strMailFrom = "noreply@c2itconsulting.net"

'who are you mailing to?  Input mode only.  Command-line parameters take 
' precedence
strMailto = "andrewroy@c2itconsulting.net"

'set SMTP email server address here
strSMTPServer = "smtp.mandrillapp.com"
...
strAuthType = "cdoBasic"

'SMTP authentication ID
strAuthID = "chetcromer@c2itconsulting.net"

'Password for the ID
strAuthPassword = "wMe3bFDgRHd6Z3qqoCA9gw"
...
If blnEmail = 1 then 
    ...
    If lcase(strAuthType) <> "cdoanonymous" Then
      ...
      Else
        writelog("SMTP Auth User ID: " & strAuthID)
        ...
        Else
          if strAuthPassword = "" then 
            writelog("You have specified a SMTP user ID, but have not specified a password.  Switching to anonymous authentication.")
            strAuthType = "cdoanonymous"
          end if
        End if
...
writelog("SMTP Port: " & iSMTPServerPort)
writelog("SMTP Auth: " & strAuthType)
writelog("SMTP User Name: " & strAuthID)
writelog("SMTP Password: " & strAuthPassword)
